...
const SCRIPT_URL   = "https://script.google.com/macros/s/AKfycbxm7drLHxRRVchRQikNAHYqS-ya_vxQ8Z0p5tiavGTmMWlZ7TLEZAyzi3e78JhULpDk6g/exec";
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.55)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#fff",borderRadius:16,padding:"28px 28px 24px",width:"100%",maxWidth:500,boxShadow:"0 24px 64px rgba(0,0,0,.22)",fontFamily:"'DM Sans',sans-serif",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:16}}>
          <button onClick={()=>count&&onConfirm(entries)} disabled={!count} style={{flex:1,padding:11,border:"none",borderRadius:8,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,color:"#fff",background:count?deptCfg.color:"#D1D5DB",cursor:count?"pointer":"not-allowed"}}>
...
function ActivityExplorer({ log, onRemove, onExportView }) {
  ...
  log.filter(e=>e.type==="External").forEach(e=>{
    const c=e.club||"Unknown";
    if(!clubMap[c])clubMap[c]={club:c,league:e.league||"",count:0,value:0};
  ...
  const staffMap={};
  log.forEach(e=>{
    if(!staffMap[e.staff])staffMap[e.staff]={staff:e.staff,count:0,value:0,ext:0,int:0};
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",background:"#F1F5F9",borderRadius:12,padding:4,gap:2}}>
          {[["club","By Club"],["dept","By Department"],["staff","By Staff"],["log","Full Log"]].map(([v,label])=>(
            <button key={v} onClick={()=>setView(v)} style={tBtn(view===v)}>{label}</button>
          ))}
        </div>
      ...
      {view==="club"&&(
        <TblWrap>
          <thead><tr><TH>Club</TH><TH>League</TH><TH right># Logged</TH><TH right>Total Value</TH><TH right>% of Total</TH></tr></thead>
          <tbody>
            {clubRows.map(c=>{
              const pct=total?c.value/total:0;
              return(<tr key={c.club}><TD bold color="#111827">{c.club}</TD><TD><LeagueBadge league={c.league||"League-wide"}/></TD><TD right>{c.count}</TD><TD right bold>{fmt$(c.value)}</TD><TD right><div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:8}}><div style={{width:60,height:6,background:"#F3F4F6",borderRadius:99,overflow:"hidden"}}><div style={{width:`${pct*100}%`,height:"100%",background:"#6366F1",borderRadius:99}}/></div>{(pct*100).toFixed(1)}%</div></TD></tr>);
            })}
          </tbody>
        </TblWrap>
      )}
      ...
      {view==="staff"&&(
        <TblWrap>
          <thead><tr><TH>Staff Member</TH><TH right># Logged</TH><TH right>Total Value</TH><TH right>External</TH><TH right>Internal</TH><TH right>% of Total</TH></tr></thead>
          <tbody>
            {staffRows.map(s=>{
              const pct=total?s.value/total:0;
              const initials=s.staff.split(" ").map(n=>n[0]).join("").slice(0,2);
              return(<tr key={s.staff}><TD bold><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:"50%",background:"#EEF2FF",color:"#4338CA",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{initials}</div>{s.staff}</div></TD><TD right>{s.count}</TD><TD right bold>{fmt$(s.value)}</TD><TD right color="#0369A1">{s.ext}</TD><TD right color="#047857">{s.int}</TD><TD right><div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:8}}><div style={{width:60,height:6,background:"#F3F4F6",borderRadius:99,overflow:"hidden"}}><div style={{width:`${pct*100}%`,height:"100%",background:"#4338CA",borderRadius:99}}/></div>{(pct*100).toFixed(1)}%</div></TD></tr>);
            })}
          </tbody>
        </TblWrap>
      )}
        ...
        <div>
          ...
          <TblWrap>
            <thead><tr>
              ...
              <TH>#</TH><TH>Time</TH><TH>Staff</TH><TH>Dept</TH><TH>Deliverable</TH><TH>Type</TH><TH>Club / Recipient</TH><TH>League</TH><TH right>Rack Rate</TH>
            </tr></thead>
            <tbody>
              {reversed.map((e,i)=>{
